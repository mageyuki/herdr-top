...
fn healthy_report() -> DoctorReportV1 {
    DoctorReportV1 {
        ...
        log_lane: LogLaneSection {
            readable: check(
                CheckStatus::Ok,
                "log_lane_roots_readable",
                Some(roots(true)),
            ),
            coverage: check(
                CheckStatus::Ok,
                "log_lane_coverage_complete",
                Some(LogLaneCoverageObservation {
                    pane_sessions_total: 2,
                    pane_sessions_with_artifacts: 2,
                    pane_sessions_without_artifacts: 0,
                    rejected_targets: 0,
                }),
            ),
            freshness: check(
                CheckStatus::Ok,
                "log_lane_fresh",
                Some(LogLaneFreshnessObservation {
                    last_watcher_observation_ms: Some(1_000),
                    age_ms: Some(0),
