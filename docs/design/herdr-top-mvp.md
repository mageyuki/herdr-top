...
- stable ordering and selection during updates: Task Runs and Agent Nodes receive unique, persisted, immutable display ordinals on first entry into the model and siblings sort by them, never by an identity-key component; topology rows use a stable in-session adoption order; a state refresh never reorders rows, and after a merge the merged-in rows vanish while the survivor keeps its own ordinal and relative position;
